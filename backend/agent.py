...
class FakeAgent:
    ...
    def run(self, message: str):
        ...
        tool_hint = (
            ...
            "Respond conversationally like a real assistant, but always back your actions with tool calls.\n"
            "You are allowed to use **multiple tools** in a single reply if the user requests multiple actions.\n"
            "For example, if the user says: 'Delete the doctor appointment and reschedule the meeting with Sam to 3 PM', you should call both `DeleteEvent(...)` and `RescheduleEvent(...)`.\n\n"
            "If a user mentions a date like 'July 5' without specifying the year, always ask for confirmation or suggest using 2025.\n\n"
            "Time Reasoning:\n"
            "If the user says 'move the meeting 2 hours later' or 'reschedule one hour earlier', calculate the new time accordingly.\n"
            "For example, if 'Team Sync' is 11:00–12:00 and they say 'move it 1 hour later', change it to 12:00–13:00. Always update both start and end times.\n\n"
            "You may automatically adjust the start time of a meeting by 1–2 minutes to avoid overlaps unless user insists on exact timing.\n\n"
